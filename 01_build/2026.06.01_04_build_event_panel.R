...
ibes   <- readRDS(file.path(build_dir, "2026.06.01_ibes_data.rds"))
taq    <- readRDS(file.path(build_dir, "2026.06.01_equity_daily_returns.rds"))
idx    <- readRDS(file.path(build_dir, "2026.06.01_index_daily_returns.rds"))
...
# OOS CHANGE (2026.06.01): primary outcome is the on-chain Dome market
# resolution (resolved_beat), which is available for ALL 321 markets and is
# the literal beat/miss the strategy pays off on. The original paper used
# (actual_eps > eps_target) from IBES; that proxy is (a) missing for most of
# the post-Feb-2026 holdout due to IBES publication lag, and (b) contaminated
# by IBES annual-vs-quarterly actuals (~11% disagreement, dominated by Q4
# fiscal-year-end names where the matched "actual" is a full-year figure).
# We retain the IBES proxy as actual_beat_ibes for the matched subset.
panel[, actual_beat_ibes        := actual_eps > eps_target]
panel[, actual_beat             := resolved_beat]
...
saveRDS(panel, file.path(build_dir, "2026.06.01_event_panel.rds"))
