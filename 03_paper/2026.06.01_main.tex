...
% --- Title ---
\title{\Large \textbf{Minority Report: Contrarian Traders, Prediction Markets,\\and the Return of Post-Earnings Drift}\thanks{Advised by Kevin Smith, PhD, Stanford Graduate School of Business. Data provided by Kurush Dubash and Kunal Roy, founders of Dome API (now part of Polymarket).}}
...
\begin{abstract}
\noindent
Prediction markets on Polymarket let traders bet on whether a company will beat or miss its EPS consensus. Using resolved markets matched to IBES forecasts, I document four findings and re-test each one out-of-sample on a second, disjoint set of markets. First, the crowd is well calibrated and beats a naive baseline on Brier score, partly because it keeps trading through the roughly three-week window after the last analyst update freezes. Second, when order flow is strongly one-sided, the outcome becomes close to certain: the intensity of conviction, not the price level, is what identifies beats and misses. Third, a short-only strategy triggered by a low crowd-assigned beat probability earns significant returns over a one- to two-week horizon, consistent with conditional post-earnings drift; the gain holds up out-of-sample as a tradeable return, but it is largely compensation for the short portfolio's market exposure, so its risk-adjusted (Fama--French three-factor) alpha is marginal rather than clean. Fourth, on-chain profiling of ``smart money'' wallets points to domain specialists and algorithmic liquidity providers rather than omniscient insiders, and the same wallets stay accurate on markets they were not selected on. The signal is most useful as a short-side filter: when the crowd assigns a low beat probability, shorting into earnings has reliably paid.
...
Q3 & 68 & 0.390 & 97.1\% \\
Q4 & 67 & 0.519 & 98.5\% \\
Q5 (most bullish) & 68 & 0.681 & 100.0\% \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
\midrule
1 (bearish) & 113 & $-$0.118 & 34.5\% & $-$0.325 & $-$0.63\% \\
2 (neutral) & 112 & 0.385 & 91.1\% & $+$0.182 & $+$0.75\% \\
3 (bullish) & 113 & 0.626 & 98.2\% & $+$0.312 & $+$0.10\% \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
The tercile sort reveals a stark gradient. In the most bearish flow tercile, only 34.5\% of events beat the analyst consensus, and the mean excess return is $-$0.63\%. In the most bullish tercile, 98.2\% beat consensus, with a mean excess return of $+$0.10\%. The mean surprise rises monotonically from $-$\$0.33 in the bearish tercile to $+$\$0.31 in the bullish tercile, so crowd flow orders events correctly in both direction and sign. The 63.7 percentage-point spread in beat rates across flow terciles demonstrates that crowd order flow contains substantial information about the direction of earnings surprises relative to analyst expectations.
...
However, the crowd's accuracy is not simply a mechanical exploitation of the walk-down. If it were, a naive strategy of always predicting ``beat'' would suffice, and the Brier score would not improve on the naive baseline (which already encodes the 74\% beat rate). The crowd's edge (BSS $= +0.33$) comes from identifying \textit{which specific events} will beat, not just that most will---and that edge lives in the crowd's \textit{probability}, not in a sharper dollar figure. Once actuals are measured on the correct quarterly basis, the consensus already sits within a nickel of realized EPS (median consensus \$1.08 versus median actual \$1.12 for non-GAAP events), leaving almost no dollar gap to close. Inverting the beat probability into an implied EPS (Section~\ref{sec:implied}) in fact \textit{overshoots} this small gap and does not improve on the consensus as a point estimate. The crowd's contribution is directional---which firms beat---not scalar.
